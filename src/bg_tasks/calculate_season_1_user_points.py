...
def calculate_season_1_user_points():
    logger.info("Calculating season 1 user points...")
    statement = select(User)
    users = session.exec(statement).all()
    for user in users:
        statement = select(UserPoints).where(UserPoints.wallet_address == user.wallet_address, UserPoints.partner_name == "Harmonix")
        user_points = session.exec(statement).all()
        total_points = 0
        for points in user_points:
            total_points += points.points
        user_season_1_points = UserSeason1Points(user_id=user.user_id, points=total_points, recorded_at=datetime.now(timezone.utc))
        session.add(user_season_1_points)
    session.commit()
    logger.info("Season 1 user points calculated successfully.")

if __name__ == "__main__":
    calculate_season_1_user_points()