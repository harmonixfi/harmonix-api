...
async def get_withdraw_quote(
    ...
    if withdraw_value > position.total_balance:
        raise HTTPException(
            status_code=400, detail="Withdraw value exceeds total balance"
        )
    statement = session.exec(
        select(UserPortfolio).where(UserPortfolio.user_address == wallet_address)
    ).all()
    total_deposit = 0
    total_balance = 0
    ...
    spot_perp_spread = (
        float(
            session.exec(
                select(ConfigQuotation.value).where(
                    ConfigQuotation.key == constants.SPOT_PERP_SPREAD
                )
            ).one()
        )
        / 100
    ...
    performance_fee = (
        float(
            session.exec(
                select(ConfigQuotation.value).where(
                    ConfigQuotation.key == constants.PERFORMANCE_FEE
                )
            ).one()
        )
        / 100
    ...
    management_fee = (
        float(
            session.exec(
                select(ConfigQuotation.value).where(
                    ConfigQuotation.key == constants.MANAGEMENT_FEE
                )
            ).one()
        )
        / 100
    ...
    management_fee *= (
        ...
        * (
            ...
            - pos.trade_start_date.replace(tzinfo=timezone.utc)
        ).days
    ) / 365
